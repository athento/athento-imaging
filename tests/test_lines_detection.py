import pytest
import os
import athentoimaging.lines_detection as ld
import numpy as np
...
class Test_LD:
    ...
    # Tests of detect_line
        ...
        def test_detect_lines_min_val_negative_error(self):
            with pytest.raises(ValueError) as exc:
                ld.detect_lines(self.test_image, -200)
    ...
    # Tests of delete_lines
        ...
        def test_delete_lines_length_negative(self):
            with pytest.raises(ValueError) as exc:
                ld.delete_lines(self.test_image, self.lines, line_length=-100)
            assert exc.value.message == "Line_length value must be greater than 0."
    ...
    # Tests of delete_all_lines
        ...
        def test_delete_all_lines_length_negative(self):
            with pytest.raises(ValueError) as exc:
                ld.delete_all_lines(self.test_image, line_length=-100)
            assert exc.value.message == "Line_length value must be greater than 0."
    ...
    # Tests of distance
        ...
        def test_distance_line_length_negative(self):
            with pytest.raises(ValueError) as exc:
                ld.distance(self.lines[0][0], self.lines[0][1], line_length=-10)
            assert exc.value.message == "Line_length value must be greater than 0."
    ...
    # Tests of distance_mean
        ...
        def test_distance_mean_line_length_negative(self):
            with pytest.raises(ValueError) as exc:
                ld.distance_mean(self.lines, line_length=-100)
            assert exc.value.message == "Line_length value must be greater than 0."
    ...
    # Tests of draw_lines
        ...
        def test_draw_lines_line_length_negative(self):
            with pytest.raises(ValueError) as exc:
                ld.draw_lines(self.test_image, self.lines[0][0], line_length=-100)
            assert exc.value.message == "Line_length value must be greater than 0."
    ...
    # Tests of get_line_coordinates

        def test_get_line_coordinates_good(self):
            assert len(ld.get_line_coordinates(self.lines[0][0])) == 4

        def test_get_line_coordinates_line_none(self):
            with pytest.raises(ValueError) as exc:
                ld.get_line_coordinates(None)
            assert exc.value.message == "Line must be a line, is None."

        def test_get_line_coordinates_line_wrong(self):
            with pytest.raises(ValueError) as exc:
                ld.get_line_coordinates([5])
            assert exc.value.message == "Wrong format on line, line must be [rho, theta]."

        def test_get_line_coordinates_line_length_negative(self):
            with pytest.raises(ValueError) as exc:
                ld.get_line_coordinates(self.lines[0][0], line_length=-100)
            assert exc.value.message == "Line_length value must be greater than 0."
    ...
    # Tests of line_count
        ...
        def test_line_count_line_length_negative(self):
            with pytest.raises(ValueError) as exc:
                ld.line_count(self.lines[0][1], line_length=-5)
            assert exc.value.message == "Line_length value must be greater than 0."
    ...
    # Tests of parallels
        ...
        def test_parallels_line_length_negative(self):
            with pytest.raises(ValueError) as exc:
                ld.parallels(self.lines[0][0], self.lines[0][1], line_length=-5)
            assert exc.value.message == "Line_length value must be greater than 0."
